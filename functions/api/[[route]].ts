...
/// WebSocket (Durable Object) 経由でチャットイベントをサーバーメンバーに配信する
async function dispatchChatEvent(
  env: { CHAT_STREAM?: DurableObjectNamespace },
  serverId: string,
  event: Record<string, unknown>,
): Promise<void> {
  if (!env.CHAT_STREAM) return;
  try {
    const doId = env.CHAT_STREAM.idFromName(serverId);
    const stub = env.CHAT_STREAM.get(doId);
    await stub.fetch('http://internal/dispatch', {
      method: 'POST',
      body: JSON.stringify(event),
    });
  } catch (e) {
    console.error('[chat] dispatchChatEvent failed:', e);
  }
}
...
app.get('/api/chat/servers', requireAuth, async (c) => {
  try {
    ...
    `)
      .bind(userId)
      .all();
    if (!result.success) return c.json({ error: 'Failed to fetch servers' }, 500);
    return c.json({ servers: result.results });
  } catch (error: unknown) {
    const err = error as { message?: string };
    console.error('Fetch chat servers error:', error);
    return c.json({ error: 'Failed to fetch servers', details: err.message || 'Unknown error' }, 500);
  }
});

// POST /api/chat/servers - Create a new server
app.post('/api/chat/servers', requireAuth, async (c) => {
  try {
    const user = c.get('user') as { id: string; username: string } | undefined;
    if (!user) return c.json({ error: 'Unauthorized' }, 401);
    if (!c.env.DB) return c.json({ error: 'Database not available' }, 500);
    const { name, description } = (await c.req.json()) as { name: string; description?: string };
    if (!name || name.trim().length === 0) return c.json({ error: 'Server name is required' }, 400);
    if (name.length > 100) return c.json({ error: 'Server name too long' }, 400);
    ...
    return c.json({
      server: { id: serverId, name: name.trim(), description: description?.trim() || '', owner_id: user.id },
    });
  } catch (error: unknown) {
    const err = error as { message?: string };
    console.error('Create chat server error:', error);
    return c.json({ error: 'Failed to create server', details: err.message || 'Unknown error' }, 500);
  }
});

// GET /api/chat/servers/:id - Get server details
app.get('/api/chat/servers/:id', requireAuth, async (c) => {
  try {
    const userId = c.get('user')?.id || '';
    ...
    const server = await c.env.DB.prepare(`
      SELECT cs.*, csm.role as my_role,
        (SELECT COUNT(*) FROM chat_server_members WHERE server_id = cs.id) as member_count
      FROM chat_servers cs
      JOIN chat_server_members csm ON cs.id = csm.server_id AND csm.user_id = ?
      WHERE cs.id = ?
    `)
    ...
    const err = error as { message?: string };
    console.error('Fetch chat server error:', error);
    return c.json({ error: 'Failed to fetch server', details: err.message || 'Unknown error' }, 500);
  }
});

// POST /api/chat/servers/:id/join - Join a server
app.post('/api/chat/servers/:id/join', requireAuth, async (c) => {
  try {
    const user = c.get('user') as { id: string; username: string } | undefined;
    if (!user) return c.json({ error: 'Unauthorized' }, 401);
    const serverId = c.req.param('id');
    ...
    const server = await c.env.DB.prepare('SELECT id FROM chat_servers WHERE id = ?').bind(serverId).first();
    if (!server) return c.json({ error: 'Server not found' }, 404);

    const existing = await c.env.DB.prepare('SELECT 1 FROM chat_server_members WHERE server_id = ? AND user_id = ?')
      .bind(serverId, user.id)
      .first();
    if (existing) return c.json({ error: 'Already a member' }, 409);

    await c.env.DB.prepare('INSERT INTO chat_server_members (server_id, user_id, role) VALUES (?, ?, ?)')
      .bind(serverId, user.id, 'member')
      .run();

    return c.json({ ok: true });
  } catch (error: unknown) {
    const err = error as { message?: string };
    console.error('Join chat server error:', error);
    return c.json({ error: 'Failed to join server', details: err.message || 'Unknown error' }, 500);
  }
});

// POST /api/chat/servers/:id/leave - Leave a server
app.post('/api/chat/servers/:id/leave', requireAuth, async (c) => {
  try {
    const userId = c.get('user')?.id || '';
    const serverId = c.req.param('id');
    if (!c.env.DB) return c.json({ error: 'Database not available' }, 500);
    ...
    const membership = (await c.env.DB.prepare(
      'SELECT role FROM chat_server_members WHERE server_id = ? AND user_id = ?',
    )
      .bind(serverId, userId)
      .first()) as { role: string } | null;
    if (!membership) return c.json({ error: 'Not a member' }, 404);
    if (membership.role === 'owner')
      return c.json({ error: 'Owner cannot leave; transfer ownership or delete the server' }, 403);

    await c.env.DB.prepare('DELETE FROM chat_server_members WHERE server_id = ? AND user_id = ?')
      .bind(serverId, userId)
      .run();
    return c.json({ ok: true });
  } catch (error: unknown) {
    const err = error as { message?: string };
    console.error('Leave chat server error:', error);
    return c.json({ error: 'Failed to leave server', details: err.message || 'Unknown error' }, 500);
  }
});

// GET /api/chat/servers/:id/channels - List channels in a server
app.get('/api/chat/servers/:id/channels', requireAuth, async (c) => {
  try {
    const userId = c.get('user')?.id || '';
    const serverId = c.req.param('id');
    if (!c.env.DB) return c.json({ error: 'Database not available' }, 500);

    const member = await c.env.DB.prepare('SELECT 1 FROM chat_server_members WHERE server_id = ? AND user_id = ?')
      .bind(serverId, userId)
      .first();
    ...
    if (!result.success) return c.json({ error: 'Failed to fetch channels' }, 500);
    return c.json({ channels: result.results });
  } catch (error: unknown) {
    const err = error as { message?: string };
    console.error('Fetch channels error:', error);
    return c.json({ error: 'Failed to fetch channels', details: err.message || 'Unknown error' }, 500);
  }
});

// POST /api/chat/servers/:id/channels - Create a channel
app.post('/api/chat/servers/:id/channels', requireAuth, async (c) => {
  try {
    const userId = c.get('user')?.id || '';
    const serverId = c.req.param('id');
    if (!c.env.DB) return c.json({ error: 'Database not available' }, 500);
    ...
    if (membership.role === 'member') return c.json({ error: 'Only admins can create channels' }, 403);

    const { name, type } = (await c.req.json()) as { name: string; type?: string };
    if (!name || name.trim().length === 0) return c.json({ error: 'Channel name is required' }, 400);
    if (name.length > 100) return c.json({ error: 'Channel name too long' }, 400);
    ...
    const maxPos = (await c.env.DB.prepare(
      'SELECT COALESCE(MAX(position), -1) + 1 as next_pos FROM chat_channels WHERE server_id = ?',
    )
      .bind(serverId)
      .first()) as { next_pos: number };

    await c.env.DB.prepare('INSERT INTO chat_channels (id, server_id, name, type, position) VALUES (?, ?, ?, ?, ?)')
      .bind(channelId, serverId, name.trim(), channelType, maxPos.next_pos)
      .run();

    return c.json({
      channel: { id: channelId, server_id: serverId, name: name.trim(), type: channelType, position: maxPos.next_pos },
    });
  } catch (error: unknown) {
    const err = error as { message?: string };
...
app.get('/api/chat/channels/:id/messages', requireAuth, async (c) => {
  try {
    const userId = c.get('user')?.id || '';
    ...
    // Verify membership
    const chan = (await c.env.DB.prepare('SELECT server_id FROM chat_channels WHERE id = ?')
      .bind(channelId)
      .first()) as { server_id: string } | null;
    if (!chan) return c.json({ error: 'Channel not found' }, 404);

    const member = await c.env.DB.prepare('SELECT 1 FROM chat_server_members WHERE server_id = ? AND user_id = ?')
      .bind(chan.server_id, userId)
      .first();
    if (!member) return c.json({ error: 'Not a member of this server' }, 403);
    ...
    let messages: { results?: Record<string, unknown>[]; success: boolean };
    if (before) {
      messages = await c.env.DB.prepare(`
        SELECT cm.*, u.username, u.display_name, u.avatar_key
        FROM chat_messages cm
        LEFT JOIN users u ON cm.user_id = u.id
        WHERE cm.channel_id = ? AND cm.created_at < (SELECT created_at FROM chat_messages WHERE id = ?)
    ...
    if (!messages.success) return c.json({ error: 'Failed to fetch messages' }, 500);
    const rows = messages.results || [];

    // Fetch reactions for these messages
    const messageIds = rows.map((r: Record<string, unknown>) => r.id);
    const reactions =
      messageIds.length > 0
        ? await c.env.DB.prepare(`
    ...
    const reactionsByMsg = new Map<string, Array<Record<string, unknown>>>();
    for (const r of (reactions.results || []) as Array<Record<string, unknown>>) {
      const mid = r.message_id as string;
      if (!reactionsByMsg.has(mid)) reactionsByMsg.set(mid, []);
      reactionsByMsg.get(mid)!.push(r);
    }

    const enriched = rows.map((msg: Record<string, unknown>) => ({
      id: msg.id,
      ...
      reactions: (reactionsByMsg.get(msg.id as string) || []).map((r) => ({
        emoji: r.emoji,
        user_id: r.user_id,
      })),
    }));
...
app.post('/api/chat/channels/:id/messages', requireAuth, async (c) => {
  try {
    const userId = c.get('user')?.id || '';
    const channelId = c.req.param('id');
    if (!c.env.DB) return c.json({ error: 'Database not available' }, 500);
    ...
    const { content, reply_to_id } = (await c.req.json()) as { content: string; reply_to_id?: string };
    if (!content || content.trim().length === 0) return c.json({ error: 'Message content is required' }, 400);
    if (content.length > 4000) return c.json({ error: 'Message too long' }, 400);

    const messageId = nanoid();
    await c.env.DB.prepare(
      'INSERT INTO chat_messages (id, channel_id, user_id, content, reply_to_id) VALUES (?, ?, ?, ?, ?)',
    )
...
app.delete('/api/chat/messages/:id', requireAuth, async (c) => {
  try {
    const userId = c.get('user')?.id || '';
    ...
    const msg = (await c.env.DB.prepare(`
      SELECT cm.id, cm.user_id, cm.channel_id, cc.server_id, cs.owner_id as server_owner_id
      FROM chat_messages cm
    ...
    const member = (await c.env.DB.prepare('SELECT role FROM chat_server_members WHERE server_id = ? AND user_id = ?')
      .bind(serverId, userId)
      .first()) as { role: string } | null;

    const isAuthor = msg.user_id === userId;
    const isMod = member && (member.role === 'owner' || member.role === 'admin');
    ...
    dispatchChatEvent(c.env as { CHAT_STREAM?: DurableObjectNamespace }, serverId, {
...
app.patch('/api/chat/messages/:id', requireAuth, async (c) => {
  try {
    const userId = c.get('user')?.id || '';
    const messageId = c.req.param('id');
    if (!c.env.DB) return c.json({ error: 'Database not available' }, 500);
    ...
    const msg = (await c.env.DB.prepare(`
      SELECT cm.user_id, cm.content, cm.channel_id, cc.server_id
      FROM chat_messages cm
      JOIN chat_channels cc ON cm.channel_id = cc.id
      WHERE cm.id = ?
    `)
      .bind(messageId)
      .first()) as { user_id: string; content: string; channel_id: string; server_id: string } | null;
    if (!msg) return c.json({ error: 'Message not found' }, 404);
    if (msg.user_id !== userId) return c.json({ error: 'Forbidden' }, 403);

    const { content } = (await c.req.json()) as { content: string };
    if (!content || content.trim().length === 0) return c.json({ error: 'Content is required' }, 400);

    await c.env.DB.prepare(
      "UPDATE chat_messages SET content = ?, edited_at = strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id = ?",
    )
      .bind(content.trim(), messageId)
      .run();

    dispatchChatEvent(c.env as { CHAT_STREAM?: DurableObjectNamespace }, msg.server_id, {
...
app.post('/api/chat/messages/:id/reactions', requireAuth, async (c) => {
  try {
    const userId = c.get('user')?.id || '';
    const messageId = c.req.param('id');
    if (!c.env.DB) return c.json({ error: 'Database not available' }, 500);
    ...
    const msg = (await c.env.DB.prepare(`
      SELECT cm.channel_id, cc.server_id
      FROM chat_messages cm
      JOIN chat_channels cc ON cm.channel_id = cc.id
      WHERE cm.id = ?
    `)
      .bind(messageId)
      .first()) as { channel_id: string; server_id: string } | null;
    if (!msg) return c.json({ error: 'Message not found' }, 404);

    // Verify membership
    const member = await c.env.DB.prepare('SELECT 1 FROM chat_server_members WHERE server_id = ? AND user_id = ?')
      .bind(msg.server_id, userId)
      .first();
    ...
    await c.env.DB.prepare('INSERT OR IGNORE INTO chat_message_reactions (message_id, user_id, emoji) VALUES (?, ?, ?)')
    ...
    return c.json({ error: 'Failed to add reaction', details: err.message || 'Unknown error' }, 500);
  }
});

// DELETE /api/chat/messages/:id/reactions - Remove a reaction
app.delete('/api/chat/messages/:id/reactions', requireAuth, async (c) => {
  try {
    const userId = c.get('user')?.id || '';
    const messageId = c.req.param('id');
    if (!c.env.DB) return c.json({ error: 'Database not available' }, 500);
    ...
    if (!emoji) return c.json({ error: 'Emoji query param is required' }, 400);
    ...
    return c.json({ error: 'Failed to remove reaction', details: err.message || 'Unknown error' }, 500);
  }
});

// POST /api/chat/channels/:id/read - Mark channel as read
app.post('/api/chat/channels/:id/read', requireAuth, async (c) => {
  try {
    const userId = c.get('user')?.id || '';
    const channelId = c.req.param('id');
    if (!c.env.DB) return c.json({ error: 'Database not available' }, 500);
    ...
    const { last_message_id } = (await c.req.json()) as { last_message_id?: string };

    await c.env.DB.prepare(`
      INSERT INTO chat_read_states (user_id, channel_id, last_read_message_id, unread_count)
      VALUES (?, ?, ?, 0)
      ON CONFLICT(user_id, channel_id)
      DO UPDATE SET last_read_message_id = excluded.last_read_message_id, unread_count = 0
    `)
      .bind(userId, channelId, last_message_id || null)
      .run();
    ...
    return c.json({ error: 'Failed to mark channel as read', details: err.message || 'Unknown error' }, 500);
  }
});
...
export async function onRequest(context: Record<string, unknown>) {
  ...
  // WebSocket チャットストリーム — サーバーごとにルーム分割
  if (url.pathname === '/api/ws/chat' && request.headers.get('Upgrade') === 'websocket') {
    const sessionToken = getSessionToken(request) || url.searchParams.get('token');
    ...
    // Verify membership
    const member = await env.DB.prepare('SELECT 1 FROM chat_server_members WHERE server_id = ? AND user_id = ?')
      .bind(serverId, session.user.id)
      .first();
    if (!member) return new Response('Forbidden', { status: 403 });

    if (!env.CHAT_STREAM) return new Response('Chat not available', { status: 503 });
    const doId = env.CHAT_STREAM.idFromName(serverId);
    const stub = env.CHAT_STREAM.get(doId);
    return stub.fetch(request);
  }
