...
import { extractZipToWvfs, serveFileFromWvfs } from './lib/wvfs-zip-server'
  ...
  try {
    ...
    if (c.env.DB) {
      try {
        ...
        if (adResult?.payload_key) {
          zipObject = await c.env.BUCKET.get(adResult.payload_key)
        }
      } catch {
    ...
    if (!zipObject) {
      ...
      for (const key of keysToTry) {
        zipObject = await c.env.BUCKET.get(key)
        if (zipObject) break
      }
    }
