...
# ワンショットでタスクを依頼（対話せずに1回だけ実行）
opencode "src/main.ts の役割を解説して"

# 複数ファイルを横断して質問
opencode "認証の流れを追ってください"
```

対話モードで起動するとプロンプト（`>`）が表示され、以下のようなやり取りができます:

```
> このプロジェクトの技術スタックを教えて
> src/components/Timeline.ts に新しいフィルターボタンを追加して
> テストを実行して
> 今の変更をコミットして
```
...
```bash
# 特定のファイルについて聞く
opencode "src/main.ts の parseCurrentRoute 関数の役割は？"

# 「このファイル」と指示する（対話モードでファイルを開いた状態）
> このファイルの責務を教えて
```
...
```bash
# ビルドエラーの解決
opencode "pnpm build が通らない。エラーを調べて修正して"

# テスト失敗の分析
opencode "テストが失敗している。原因を特定して修正案を出して"
```

#### コードレビューを依頼する

```bash
opencode "直近のコミットの差分をレビューして"
```

#### 複数ファイルにまたがるタスク

```bash
opencode "新しい設定ページを作りたい。既存の SettingsPage.ts を参考に、src/components/ に新しいファイルを作成して"
```
...
```bash
# Tauri（デスクトップアプリ）
npm tauri:build          # 現在のプラットフォーム
npm tauri:build:linux    # Linux
npm tauri:build:macos    # macOS
npm tauri:build:windows  # Windows

# Capacitor（モバイルアプリ）
npm cap:sync             # Capacitor 同期
npm cap:build:android    # Android APK
npm cap:build:ios        # iOS
```
...
```text
1. npm lint を実行 → Biome のエラーを確認
2. npm lint:fix で自動修正を試みる
3. npm typecheck で型エラーがないか確認
4. npm test でテストが通るか確認
5. npm build でビルドが通るか確認
```
...
| エラー | 原因 | 対処 |
|--------|------|------|
| `biome check` が失敗 | コードスタイル違反 | `npm lint:fix` で自動修正 |
...
| `D1` 関連のエラー | マイグレーション未実行 | `npm migrate:local` を実行 |
